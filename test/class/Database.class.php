...
class Database{
	...
	public function convertCity($cityID) {
	    $cityName      =   '';
	    $cityData      =   array();
	    $this->setTable('cities');
	    if(!empty($cityID) && trim($cityID) != '') {
	        $query         =   "SELECT `city_slug` FROM `{$this->table}` WHERE `city_id` = '{$cityID}'";
	        $cityData      =   $this->listRecord($query);   
	        $cityName      =   $cityData[0]['city_slug'];
	        $cityName      =   str_replace('-', ' ', $cityName);
	    }
	    return ucwords($cityName);
	}
	
    public function convertDistrict($districtID) {
	    $districtName      =   '';
	    $districtData      =   array();
	    $this->setTable('districts');
	    if(!empty($districtID) && trim($districtID) != '') {
	        $query         =   "SELECT `district_slug` FROM `{$this->table}` WHERE `district_id` = '{$districtID}'";
	        $districtData      =   $this->listRecord($query);   
	        $districtName      =   $districtData[0]['district_slug'];
	        $districtName      =   str_replace('-', ' ', $districtName);
	    }
	    return ucwords($districtName);
	}
	
	public function convertWard($wardID) {
	    $wardName      =   '';
	    $wardData      =   array();
	    $this->setTable('wards');
	    if(!empty($wardID) && trim($wardID) != '') {
	        $query         =   "SELECT `ward_slug` FROM `{$this->table}` WHERE `ward_id` = '{$wardID}'";
	        $wardData      =   $this->listRecord($query);
	        $wardName      =   $wardData[0]['ward_slug'];
	        $wardName      =   str_replace('-', ' ', $wardName);
	    }
	    return ucwords($wardName);
	}
}